...
class LaserImage(HelixImage):
    ...
    def startAnalysis(self):
        ...
        self.lineData()
    #Analysis rotated +90 degrees for sensors in which the lines appear vertical in the images.
    def startAnalysis90(self):
        self.image = Image.open(self.location)
        self.pixel = self.image.load()
        for y in range(0,int(self.rows)):
            startPix = None
            ...
            if peakRange == None:
                continue
            for x in range(0,int(self.columns)):
                if peakRange[0] <= x <= peakRange[1]:
                    #[(x-coord,value)]
                    value = self.pixel[x,y]
                    #Gather relevant pixels in col
                    if value > threshold and startPix == None:
                        startPix = x
                        pixels.append((x,value))
                    elif value < threshold and startPix != None and endPix == None:
                        endPix = x-1
                    elif value > threshold and endPix == None:
                        pixels.append((x,value))
                        ...
                        endPix = None
                        pixels.append((x,value))
                    if endPix == None and x == int(self.columns)-1:
                        continue
                if x == int(self.columns)-1:
                    #Generate CG and line width, add to list
                    if peak > ((self.thresholdPercent/100) * 255):
                        cg = self.getCG(pixels,startPix)
                        self.CGs.append(cg)
                        ...
                        self.scores.append(focusScore)
                        self.widths.append(len(pixels))
                        self.logLines.append(f'{cg}\t{y}\t{len(pixels)}\t{peak}\t{focusScore}\t{pixels}\n')
        self.lineData()
    ...
    def lineData(self):
        ...
        lineLength = 0
        if self.rotated:
            lineLength = (int(self.rows)*0.25)
        else:
            lineLength = (int(self.columns)*0.25)
        if len(self.CGs) < lineLength:
            ...
            #Angle and focus
            if self.rotated:
                self.lineAngle = round(math.degrees(math.atan((self.CGs[-1] - self.CGs[0])/self.rows)),2)
            else:
